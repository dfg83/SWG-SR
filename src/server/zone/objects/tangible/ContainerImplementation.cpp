...
void ContainerImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	...
	if (checkContainerPermission(player, ContainerPermissions::MOVECONTAINER && getParent().get() != NULL &&
			getParent().get()->checkContainerPermission(player, ContainerPermissions::MOVEOUT) && !(_this.getReferenceUnsafeStaticCast()->isRecycleToolObject()) && !(_this.getReferenceUnsafeStaticCast()->isAntiDecayKitObject()) && (_this.getReferenceUnsafeStaticCast()->getObjectTemplate()->getFullTemplateString() != "object/tangible/item/duplicator_prototype.iff"))) {
		if (_this.getReferenceUnsafeStaticCast()->getObjectTemplate()->getFullTemplateString() == "object/tangible/item/duplicator_prototype.iff") {
				return;
		}
