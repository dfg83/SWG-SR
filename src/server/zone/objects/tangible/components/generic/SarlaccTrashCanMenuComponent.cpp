...
int SarlaccTrashCanMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectedID) const {
	...
	if (selectedID == 20) {
		...
		if (itemCount > 10) {
			player->sendSystemMessage("Sarlacc Trash Can cannot comsume more than 10 items at a time.");
			return 0;
		}
		...
		if (noTrade != 0) {
			box->setPromptTitle("Sarlacc Trash Can: Undeletable Items");
			text = "\\#ff0000[ATTENTION] \\#eeeeeeThe following items cannot be deleted by the Sarlacc Trash Can.\n\n";
			text += noTradeText;
