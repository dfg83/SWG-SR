...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!shuttle->isInRange(creature, 128.f)) {
		...
		// No wait time for Tansarii either.
		if (!closestPoint->isPoint("naboo","Theed Spaceport") || !closestPoint->isPoint("dungeon","Tansarii Station")){
		...
		if (departCity != NULL){
			...
			if (departCity->getMayorID() != 0) {
				if (!shuttle->isInRange(creature, 52.f)) {
					creature->sendSystemMessage("@player_structure:boarding_too_far"); //You are too far from the shuttle to board.
					return GENERALERROR;
				}
				if (creature->getParent() != NULL) {
					creature->sendSystemMessage("You cannot board a player city shuttle from inside of a structure.");
					return GENERALERROR;
				}
			}
		}
		...
		// Ports the player into the structure when landing on Tansarii. Planet Config Manager is set to world position of the shuttle area.
		if (arrivalZone->getZoneName() == "dungeon2") {
			creature->switchZone(arrivalZone->getZoneName(), 53.5, 0.8, 27.6, 14200836);
		} else {
