...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
		...
		if (!freeGodMode) {
			try {
				...
				if(accountPermissionLevel >= 0 ) {
					...
					/*Reference<ShipControlDevice*> shipControlDevice = zoneServer->createObject(STRING_HASHCODE("object/intangible/ship/shared_z95_pcd.iff"), 1).castTo<ShipControlDevice*>();
					//ShipObject* ship = (ShipObject*) server->createObject(STRING_HASHCODE("object/ship/player/shared_player_z95.iff"), 1);
					Reference<ShipObject*> ship = zoneServer->createObject(STRING_HASHCODE("object/ship/player/shared_player_z95.iff"), 1).castTo<ShipObject*>();
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						query << "SELECT UNIX_TIMESTAMP(creation_date) FROM characters c WHERE galaxy_id = " << galaxyId << " AND account_id = " << accountId << " ORDER BY creation_date desc;";
						...
						if (res != NULL && res->next()) {
							...
							//if (timeVal.miliDifference() < 43200000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 6 hours. Repeat attempts prior to 6 hour elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 21600000) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 6 hours. Repeat attempts prior to 6 hour elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
		...
		try {
			...
			if (res != NULL && res->next()) {
				...
				if (charactersTotal >= 5) {
					// Has created more than 5 characters, give default credits
					playerCreature->setCashCredits(100, false);
					playerCreature->setBankCredits(1000, false);
				}
			}
		} catch (DatabaseException& e) {
			error(e.getMessage());
	...
	box->setPromptText("You are limited to creating one character per 6 hour period. Attempting to create another character or deleting your character before the 6 hour timer expires will reset the timer.\n\nPlease be aware, only your first 5 characters on Sentinels Republic are eligible to recieve 110,000 credits. After that your characters will only recieve 1100 credits.");
