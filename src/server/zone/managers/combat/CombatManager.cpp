...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* targetCreature, WeaponObject* weapon, const CreatureAttackData& data, int damage, int accuracyBonus) {
	...
	if (damage > 0) {
		...
		if (def == "saber_block") {
			...
			if (saberBlock <= 1) // Saber Block is too low to matter
				return HIT;

			if (attacker->isPlayerCreature()) {
				if (saberBlock > 70) // 70% max PvP block chance
					saberBlock = 70;
				if (targetCreature->hasState(CreatureState::STUNNED)) // PvP stunned, -20% saber block
					saberBlock -= 20;
				if (targetCreature->hasState(CreatureState::BLINDED)) // PvP blinded, -10% saber block
					saberBlock -= 10;
			} else {
				if (saberBlock > 85) // 85% max PvE block chance
					saberBlock = 85;
				if (targetCreature->hasState(CreatureState::STUNNED)) // PvE stunned, -10% saber block
					saberBlock -= 10;
				if (targetCreature->hasState(CreatureState::BLINDED)) // PvE blinded, -5% saber block
					saberBlock -= 5;
			}
			...
			if (!attacker->isTurret() && (weapon->getAttackType() == SharedWeaponObjectTemplate::RANGEDATTACK) && ((System::random(100)) < saberBlock))
				return RICOCHET;
			else return HIT;
		}
