...
narmle_racetrack_screenplay = RaceTrack:new {
	trackConfig={
		...
		BARCComponent="object/tangible/component/vehicle/fuel_cell_a.iff",  -- BARC Component to be awarded
...
narmle_racetrack_convo_handler = conv_handler:new {}

function narmle_racetrack_convo_handler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	local screen = LuaConversationScreen(pConvScreen)
	local screenID = screen:getScreenID()
	if ( screenID == "cs_jsPlumb_1_116" ) then
		narmle_racetrack_screenplay:startRacing(pPlayer)
	elseif ( screenID == "cs_jsPlumb_1_181" ) then -- Personal Best
		narmle_racetrack_screenplay:displayPersonalBestTime(pPlayer)
	elseif ( screenID == "cs_jsPlumb_1_207" ) then -- Track Best
		narmle_racetrack_screenplay:displayTrackBestTime(pPlayer)
	end
	return pConvScreen
end
